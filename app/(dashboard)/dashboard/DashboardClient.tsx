...
interface StudentWithPlan extends Student {
  training_plans: { id: string; division_type: string; total_weeks: number; active: boolean }[]
}
...
export default function DashboardClient({ trainer, initialStudents }: Props) {
  ...
  const [form, setForm] = useState({
    ...
    goal: '', level: 'intermediário' as Student['level'],
  })

  const FREE_LIMIT = 3
  const isFreeLimited = trainer.plan === 'free' && students.length >= FREE_LIMIT
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    setShowModal(false)
    setForm({ name: '', age: '', weight: '', height: '', goal: '', level: 'intermediário' })
    setSaving(false)
    ...
    <>
      ...
      <div className="container">
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px', flexWrap: 'wrap', gap: '12px' }}>
          <div>
            <h1 style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: '2.2rem', color: 'var(--primary)', letterSpacing: '2px', lineHeight: 1 }}>
              ALUNOS
            </h1>
            <p className="text-muted text-sm" style={{ marginTop: '4px' }}>
              {students.length} aluno{students.length !== 1 ? 's' : ''}
              {trainer.plan === 'free' && ` · plano gratuito (${students.length}/${FREE_LIMIT})`}
            </p>
          </div>
          ...
          {isFreeLimited ? (
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              ...
              <Link href="/dashboard/settings#upgrade" className="btn btn-primary btn-sm">Fazer upgrade</Link>
            </div>
          ) : (
        ...
        {students.length === 0 ? (
          <div className="card" style={{ padding: '48px', textAlign: 'center' }}>
            <p style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: '1.4rem', color: 'var(--muted)', letterSpacing: '2px' }}>
              Nenhum aluno ainda
            </p>
                  ...
                  <div className="card card-red-top" style={{ padding: '18px', cursor: 'pointer', transition: 'border-color 0.15s' }}>
                    ...
                    {(s.age || s.weight || s.level) && (
                      <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', marginBottom: '10px' }}>
                        ...
                        {s.weight && <span className="badge">{s.weight}kg</span>}
                    ...
                    {activePlan ? (
                      <div style={{ borderTop: '1px solid var(--border)', paddingTop: '10px', marginTop: '4px' }}>
                        <span className="badge badge-blue">{activePlan.division_type}</span>
                        <span className="text-muted text-xs" style={{ marginLeft: '6px' }}>
                          {activePlan.total_weeks} semanas
                        </span>
                      </div>
                    ) : (
                      <p className="text-muted text-xs" style={{ borderTop: '1px solid var(--border)', paddingTop: '10px' }}>
                        Sem plano de treino
                      </p>
                    )}

                    <p className="text-xs" style={{ marginTop: '8px', color: 'var(--primary)' }}>
                      /{trainer.slug}/{s.slug}
                    </p>
                  </div>
                  </Link>
          ...
          <div className="modal">
            ...
            <form onSubmit={handleCreate} style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              ...
              <div className="form-group">
                ...
                <input name="name" value={form.name} onChange={handleChange} placeholder="Maria Silva" required />
              </div>

              <div className="form-grid-3">
                <div className="form-group">
                  <label className="form-label">Idade</label>
                  <input name="age" type="number" min="10" max="99" value={form.age} onChange={handleChange} placeholder="28" />
                </div>
                <div className="form-group">
                  <label className="form-label">Peso (kg)</label>
                  <input name="weight" type="number" step="0.1" min="30" max="300" value={form.weight} onChange={handleChange} placeholder="70.0" />
                </div>
                <div className="form-group">
                  <label className="form-label">Altura (m)</label>
                  <input name="height" type="number" step="0.01" min="1.2" max="2.5" value={form.height} onChange={handleChange} placeholder="1.70" />
                </div>
              </div>
              ...
              <div className="form-group">
                ...
                <input name="goal" value={form.goal} onChange={handleChange} placeholder="Hipertrofia + Fat Loss" />
              </div>
