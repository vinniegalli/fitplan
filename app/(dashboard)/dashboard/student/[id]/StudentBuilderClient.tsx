...
import type { Trainer, Student, TrainingPlan, WorkoutDay, Exercise, PeriodizationWeek } from '@/lib/types'
...
export default function StudentBuilderClient({
  trainer, student,
  initialPlan, initialDays, initialExercises, initialPeriodization,
}: Props) {
  const supabase = createClient()

  const [plan, setPlan]       = useState<TrainingPlan | null>(initialPlan)
  const [days, setDays]       = useState<WorkoutDay[]>(initialDays)
  const [exercises, setExercises] = useState<Record<string, Exercise[]>>(initialExercises)
  const [periods, setPeriods] = useState<PeriodizationWeek[]>(initialPeriodization)
  const [tab, setTab]         = useState<Tab>('treino')
  const [activeDay, setActiveDay] = useState<string | null>(initialDays[0]?.id ?? null)
  ...
  const [addExType, setAddExType]     = useState<Exercise['type']>('compound')
  ...
  async function handleCreatePlan() {
    ...
    if (error || !newPlan) { setCreatingPlan(false); return }

    // Create workout days from template
    const dayRows = tpl && tpl.type !== 'Custom'
      ? tpl.days.map((d, i) => ({
          plan_id: newPlan.id,
          day_code: d.code,
          label: d.label,
      ...
      : [{ plan_id: newPlan.id, day_code: 'A', label: 'Treino A', focus: '', warmup: '', day_order: 0 }]
  ...
  async function handleAddExercise(e: React.FormEvent) {
    ...
    const { data: newEx } = await supabase
      ...
      .insert({
        ...
        name: addExName.trim(),
        type: addExType,
        cluster_block: addExType === 'cluster' ? (addExBlock || '(3×3)') : null,
    ...
    if (newEx) {
      setExercises(prev => ({ ...prev, [activeDay]: [...(prev[activeDay] ?? []), newEx] }))
      setAddExName(''); setAddExBlock(''); setAddExNotes(''); setAddExType('compound'); setAddExRest('2 min')
    }
    setSavingEx(false)
  }
    ...
    return (
      <>
        ...
        <div className="container" style={{ maxWidth: '560px' }}>
          <div className="section-label" style={{ marginTop: '8px' }}>Configurar Plano de Treino</div>
          <div className="card card-red-top" style={{ padding: '28px', display: 'flex', flexDirection: 'column', gap: '18px' }}>
            <div className="form-group">
              ...
              {template && template.type !== 'Custom' && (
                <span className="form-hint">
                  {template.sessions} sessões/semana · {template.days.map(d => d.label).join(', ')}
                </span>
              )}
            </div>
            ...
            <div className="form-group">
              ...
              <input
                type="number" min="4" max="52" value={totalWeeks}
                onChange={e => setTotalWeeks(parseInt(e.target.value))}
              />
              <span className="form-hint">A periodização padrão será criada automaticamente</span>
            </div>
      ...
      <header className="site-header">
        <div className="site-header-inner">
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
            <Link href="/dashboard" className="btn btn-ghost btn-sm">← Dashboard</Link>
            <div className="logo" style={{ fontSize: '1.4rem' }}>FIT<span>PLAN</span></div>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap' }}>
            <span className="badge">{student.name}</span>
            {plan && <span className="badge badge-blue">{plan.division_type} · {plan.total_weeks}sem</span>}
            <a
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '200px 1fr', gap: '16px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
              ...
              {days.map(d => (
                ...
                >
                  <div>{d.label}</div>
                  {d.focus && <div style={{ fontSize: '0.7rem', color: 'var(--muted)', fontWeight: 400, marginTop: '2px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{d.focus}</div>}
                ...
                <>
                  ...
                  <div className="form-group" style={{ marginBottom: '16px' }}>
                    ...
                    <textarea
                      ...
                      onBlur={e => updateWarmup(currentDay.id, e.target.value)}
                          ...
                          <tr key={ex.id}>
                            ...
                            <td>
                              <button
                                ...
                                onClick={() => handleDeleteExercise(currentDay.id, ex.id)}
                                title="Remover"
                              >
                                ✕
                              </button>
                  ...
                  {/* Add exercise form */}
                  <div className="card card-red-top" style={{ padding: '16px' }}>
                    <div className="section-label" style={{ margin: '0 0 12px' }}>Adicionar Exercício</div>
                    <form onSubmit={handleAddExercise} style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                      <div className="form-grid-2">
                        <div className="form-group">
                          ...
                          <input value={addExName} onChange={e => setAddExName(e.target.value)} placeholder="Ex: Supino Inclinado com Halteres" required />
                        </div>
                        <div className="form-group">
                      ...
                      <div className="form-grid-2">
                        {addExType === 'cluster' && (
                          <div className="form-group">
                            ...
                            <input value={addExBlock} onChange={e => setAddExBlock(e.target.value)} placeholder="(3×3)" />
                          </div>
                        )}
                        <div className="form-group">
                          <label className="form-label">Descanso</label>
                          <input value={addExRest} onChange={e => setAddExRest(e.target.value)} placeholder="2 min" />
                        </div>
                      </div>
                      <div className="form-group">
                        <label className="form-label">Notas</label>
                        <input value={addExNotes} onChange={e => setAddExNotes(e.target.value)} placeholder="Amplitude completa, RIR 1–2..." />
                      </div>
                      <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                        <button type="submit" className="btn btn-primary btn-sm" disabled={savingEx}>
                        ...
                        <>
                          <td>
                            <select
                              ...
                              onChange={e => setPeriodForm(p => ({ ...p, volume_label: e.target.value }))}
                              style={{ minWidth: '80px' }}
                            >
                              {['Deload','Baixo','Médio','Médio-Alto','Alto','Super Alto'].map(v => (
                          ...
                          <td>
                            <select
                              ...
                              onChange={e => setPeriodForm(p => ({ ...p, intensity_label: e.target.value }))}
                              style={{ minWidth: '70px' }}
                            >
                          ...
                          <td>
                            <input
                              ...
                              onChange={e => setPeriodForm(p => ({ ...p, sets_range: e.target.value }))}
                              style={{ width: '60px' }}
                            />
                          </td>
                          <td>
                            <input
                              ...
                              onChange={e => setPeriodForm(p => ({ ...p, reps_range: e.target.value }))}
                              style={{ width: '70px' }}
                            />
                          </td>
                          <td>
                            <input
                              ...
                              onChange={e => setPeriodForm(p => ({ ...p, is_cluster: e.target.checked }))}
                          ...
                          <td>
                            <input
                              ...
                              onChange={e => setPeriodForm(p => ({ ...p, observation: e.target.value }))}
                              style={{ minWidth: '160px' }}
                            />
                          </td>
                          <td>
                            <button className="btn btn-primary btn-sm" onClick={() => savePeriodWeek(pw.id)}>✓</button>
                          </td>
                        </>
                      ) : (
                        <>
                          <td>
                            <span className={
                              pw.volume_label?.toLowerCase().includes('deload') ? 'pill pill-deload' :
                              pw.volume_label?.toLowerCase().includes('alto')   ? 'pill pill-alto' :
                              pw.volume_label?.toLowerCase().includes('médio')  ? 'pill pill-medio' : 'pill pill-baixo'
                            }>{pw.volume_label ?? '—'}</span>
                          </td>
                          <td>
                            <span className={
                              pw.intensity_label === 'Alta' ? 'pill pill-alto' :
                              pw.intensity_label === 'Média' ? 'pill pill-medio' : 'pill pill-baixo'
                            }>{pw.intensity_label ?? '—'}</span>
                          </td>
                          <td>{pw.sets_range}</td>
                          <td>{pw.reps_range}</td>
                          <td>{pw.is_cluster ? <span className="badge badge-blue">Sim</span> : '—'}</td>
                          <td style={{ textAlign: 'left', maxWidth: '200px', fontSize: '0.78rem', color: 'var(--muted)' }}>{pw.observation}</td>
                          <td><span className="text-xs text-muted">editar</span></td>
                        </>
                      )}
                    </tr>
                  ))}
